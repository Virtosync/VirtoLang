...
- **General:**
  - `print`, `len`, `str`, `int`, `type`, `input`, `range`, `sum`, `min`, `max`, `abs`, `sorted`, `reverse`, `append`, `pop`, `slice`, `set`, `tuple`, `dict`, `dict_get`, `dict_set`, `dict_keys`, `dict_values`, `open`, `read`, `write`, `close`, `argv`, `help`, `exit`
- **Math:**
...
- **GUI (tkinter):**
  - `tk_root`, `tk_label`, `tk_button`, `tk_messagebox`, `tk_mainloop`
